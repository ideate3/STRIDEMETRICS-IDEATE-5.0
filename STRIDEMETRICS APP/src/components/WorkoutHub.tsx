  ...
  const handleGenerateCustomRoutine = async () => {
    ...
    try {
      ...
      setErrorInfo({
        ...
        title: isOffline ? 'Network Connection Lost' : 'Plan Generation Interrupted',
        whatHappened: isOffline
          ? 'Your connection flickered while building the workout routine.'
          : (err.message || 'An unexpected error occurred during generation.'),
        howToFix: [
          'Verify your internet connection.',
          'Tap Build Workout again to retry.',
        ],
      });
    } finally {
